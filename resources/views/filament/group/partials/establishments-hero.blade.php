...
>
    @php
        // Le nombre d'ACTIFS n'est mentionne que s'il differe du total : sinon
        // « 4 etablissements · 4 actifs » ajouterait du bruit pour rien.
        $actifs = (int) ($context['establishment_actifs'] ?? $count);
    @endphp
    <x-slot:badges>
        <span class="gp-hero-chip">{{ $count }} {{ \Illuminate\Support\Str::plural('établissement', $count) }}</span>
        @if($actifs !== $count)
            <span class="gp-hero-chip">{{ $actifs }} {{ \Illuminate\Support\Str::plural('actif', $actifs) }}</span>
        @endif
    </x-slot:badges>
